...
(function ($) {
  $.extend({
    ...
    table: {
      ...
      exportRowExcel: function (params, url, title) {
        $.modal.confirm("确定导出" + title + "吗？", function () {
          $.modal.loading("正在导出数据，请稍后...");
      ...
      selectRows: function () {
        ...
        var id = table.options.columns[1].field;
        if ($.common.isNotEmpty(table.options.rememberSelected) && table.options.rememberSelected) {
          var selectedRows = table.rememberSelecteds[table.options.id];
          if ($.common.isNotEmpty(selectedRows) && selectedRows != undefined) {
            rows = rows.concat(selectedRows);
          }
        }
        return $.common.uniqueJsonArray(rows, id);
      },
      // 回显数据字典
    ...
    modal: {
      ...
      // 弹出层指定宽度
      openPrint: function (title, url, width, height, callback) {
        //如果是移动端，就使用自适应大小弹窗
        if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
          width = 'auto';
          height = 'auto';
      ...
      openCloseFull: function (title, url, width, height) {
        if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
          width = 'auto';
          height = 'auto';
        }
      ...
      openClose: function (title, url, width, height) {
        //如果是移动端，就使用自适应大小弹窗
        if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
          width = 'auto';
          height = 'auto';
    ...
    operate: {
      ...
      // 确认提交
      confirmSubmit: function (url, confirm, data) {
        table.set();
      ...
      view: function (id) {
        ...
        if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
          ...
          $.modal.openClose("查看" + table.options.modalName, url);
        } else {
          $.modal.openClose("查看" + table.options.modalName, $.operate.viewUrl(id));
        }
      },
      // 修改信息，以tab页展现
      editTab: function (id) {
      ...
      saveBody: function (url, data, callback) {
        var config = {
          url: url,
          type: "post",
          dataType: "json",
